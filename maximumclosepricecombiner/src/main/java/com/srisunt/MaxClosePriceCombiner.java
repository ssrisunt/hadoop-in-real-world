...
public class MaxClosePriceCombiner {

    public static void main(String[] args) throws Exception {
        if (args.length != 2) {
            System.err
                    .println("Usage: MaxClosePrice <input path> <output path>");
            System.exit(-1);
        }

        // Define MapReduce job
        Job job = Job.getInstance();
        job.setJarByClass(MaxClosePriceCombiner.class);
        ...
        // Combiner (Optional) if not impact on reduce outcome
